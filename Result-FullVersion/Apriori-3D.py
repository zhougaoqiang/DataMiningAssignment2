...
def getKnItemsetDictFor3DData(data3D, knItemsetList, supportCount):
    itemDict = {}
    newKnItemsetList = []
    for itemset in knItemsetList:
        count = 0
        for data2D in data3D :
            itemset_set = set(itemset)
            count += sum(1 for transaction in data2D if itemset_set.issubset(transaction))
        if count > supportCount:
            itemDict[tuple(itemset)] = count
            newKnItemsetList.append(itemset)
    return itemDict, newKnItemsetList
...
def Apriori(filename, support, confidence=0):
    ...
    while itemsetLen > 1:
        ...
        knItemDict, knItemsetList = getKnItemsetDictFor3DData(data3D[k-1:], knItemsetList, supportCount)
...
if __name__ == '__main__':
    Apriori('Data/Bakery-formatted-number-TOP200.csv', 0.005)
