...
def saveTofile(transactions, file):
    print(file)
    with open(file, 'w+', newline='') as f:
        ...
        if (tranLen > 5000) :
            tranLen = 5000   ###get 5000 transactions only.
        else:
            print('the dataset is not meet the requirement, please check')
            exit() 
        while i < tranLen:
...
for top in range(start, end, step) :
    ...
    saveTofile(updatedTransactions, savedFilename)
